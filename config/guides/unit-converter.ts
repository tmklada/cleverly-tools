...
const guide: ToolGuide = {
  slug: "unit-converter",
  intro: [
    "A unit converter takes the guesswork out of switching between measurement systems, whether you're reading a recipe in milliliters and only own cups, checking a European shoe size, or converting a US contractor's square-foot quote into square meters for a client abroad. Instead of memorizing dozens of conversion factors or hunting down the right formula each time, this tool covers five of the most commonly needed categories, length, weight, volume, area, and speed, in one place.",
    "This unit conversion calculator is built for students checking homework, travelers converting distances and luggage weights, cooks scaling recipes between metric and US customary measures, and professionals in construction, shipping, or manufacturing who move between metric and imperial units daily. Every category supports both metric (SI) units like meters and kilograms and imperial or US customary units like feet and pounds, so it works no matter which system you're starting from.",
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "One limitation worth knowing: this tool does not include a temperature converter, since temperature scales use an offset formula (like Celsius to Fahrenheit) rather than a simple multiplication factor, and it doesn't include a dedicated time converter either. For those, a specialized converter that handles the different formula type is the better choice.",
      ],
    },
  ],
  useCases: [
  ...
  mistakes: [
    ...
    { title: "Using this tool for temperature", description: "Temperature conversion needs an offset formula (like °F = °C × 9/5 + 32), not a multiplication factor, so it isn't included in this length/weight/volume/area/speed converter." },
  ],
  tips: [
