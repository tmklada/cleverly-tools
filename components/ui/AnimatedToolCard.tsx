...
export default function AnimatedToolCard({ tool, index = 0 }: AnimatedToolCardProps) {
  return (
    <motion.div
      ...
      transition={{
        ...
        ease: EASING.default,
        delay: Math.min(index * 0.05, 0.4), // cap at 400ms total stagger
      }}
      whileHover={{ y: -4 }}
    >
      <Link
        href={`/tools/${tool.slug}`}
        className="group relative flex flex-col gap-3 p-5 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-lg transition-colors duration-200 h-full"
      >
        {/* Badges */}
