...
export default function Base64Tool() {
  ...
  function encode() {
    ...
    catch { setError("Encoding failed. Make sure the input is valid text."); }
  }

  function decode() {
    try { setOutput(decodeURIComponent(escape(atob(input.trim())))); setError(""); }
    catch { setError("Decoding failed. Make sure the input is a valid Base64 string."); }
  }
  ...
  return (
    <div className="space-y-4">
      <textarea value={input} onChange={e => { setInput(e.target.value); setError(""); }}
        placeholder="Enter text to encode, or Base64 to decode..." rows={5}
